...
public class ChipaiObj{
    ...
    public string firstChi;
    public string secondChi;
    public string threeChi;
}
public class PlayerHuaTaiTiao
{
    private static PlayerHuaTaiTiao instance;
    public string plyaerData;
    public Boolean isQuickStart=false;
    public PlayerHuaTaiTiao()
    {

    }
    public static PlayerHuaTaiTiao getMe()
    {
        if (instance == null)
        {
            instance=new PlayerHuaTaiTiao();
        }
        return instance;
    }
...
public class SignalOverItemScript : MonoBehaviour {
    ...
    public GameObject gangParther;
    public Text taishu, huashu, tiaoshu;
    ...
    private string[] baibangangList;//杠头白板杠列表
    ...
    public int gui;
    public void SetGangTouBaiBanGang(HupaiResponseItem itemData, List<AvatarVO> avatar)
    {
       // gui = GlobalDataScript.getInstance().gui;
        //for (int i = 0; i < avatar.Count; i++)
        //{
        //    int uuid = itemData.uuid;
        //    if (uuid == avatar[i].account.uuid)
        //    {
        //        GameObject itemTemp;
        //        for (int j = 0; j < avatar[i].baibangangCard.Length; j++)
        //        {
        //            if (avatar[i].baibangangCard[j] != -1)
        //            {
        //                itemTemp = Instantiate(Resources.Load("Prefab/ThrowCard/TopAndBottomCard")) as GameObject;
        //                itemTemp.transform.parent = gangParther.transform;
        //                itemTemp.transform.localScale = Vector3.one * 0.8f;
        //                itemTemp.GetComponent<TopAndBottomCardScript>().setPoint(avatar[i].baibangangCard[j]);
        //            }
    ...
    public void setUI(HupaiResponseItem itemData,List<int> validMasParm ,int mainuuid)
    {
        ...
        mainuuid = itemData.uuid;
      //  setTypeForPanle(mainuuid);

        string[] _huataitiaoData = PlayerHuaTaiTiao.getMe().plyaerData.Split(',');

        if (mainuuid == int.Parse(_huataitiaoData[0]))
        {
            huashu.text = _huataitiaoData[1];
            taishu.text = _huataitiaoData[2];
            tiaoshu.text = _huataitiaoData[3];
        }
        else if (mainuuid == int.Parse(_huataitiaoData[4]))
        {
            huashu.text = _huataitiaoData[5];
            taishu.text = _huataitiaoData[6];
            tiaoshu.text = _huataitiaoData[7];
        }
        else if (mainuuid == int.Parse(_huataitiaoData[8]))
        {
            huashu.text = _huataitiaoData[9];
            taishu.text = _huataitiaoData[10];
            tiaoshu.text = _huataitiaoData[11];
        }
        else if (mainuuid == int.Parse(_huataitiaoData[12]))
        {
            huashu.text = _huataitiaoData[13];
            taishu.text = _huataitiaoData[14];
            tiaoshu.text = _huataitiaoData[15];
        }
        ...
        totalScroe.text = itemData.totalScore+"";//全局盈亏
        gangScore.text = itemData.gangScore+""; //本局盈亏
...
*/
	...
	private void analysisPaiInfo(HupaiResponseItem parms){
	    ...
	    string gangtouStr = itemData.gangtou;
	    if (gangtouStr!=null&&gangtouStr.Length>0)
	    {
	        baibangangList = gangtouStr.Split(',');
	    }
        ...
        //1,2,3 这里需要吴哥配合处理 多加个,
		string chipaiStr = itemData.chi;
		if (chipaiStr != null && chipaiStr.Length > 0)
		{
		   // string[] chitemps = chipaiStr.Substring(0, chipaiStr.Length - 1).Split(',');
		    string[] chitemps = chipaiStr.Split(','); //1,2,3,4,5,6
		    ...
		    {
		        inedx++;
		        strtemp[inedx - 1] = chitemps[i];
		        if (inedx % 3 == 0)
		        {
		            ChipaiObj chipaiObj = new ChipaiObj();

		            inedx = 0;
		            chipaiObj.cardPionts = strtemp;
	...
	private void arrangePai(){
		...
		for(int i=0 ; i<paiArray.Length ;i++){
			...
			if (paiArray [i] > 0) {
				...
				for (int j = 0; j < paiArray [i]; j++) {
				    ...
				    if (itemTemp.GetComponent<TopAndBottomCardScript> ().getPoint()==gui)
				    {
				        itemTemp.GetComponent<TopAndBottomCardScript> ().caishenImg.gameObject.SetActive(true);
				    }
					subPaiConut += 1;
				}
        ...
        if (baibangangList!=null)
	    {
	        for (int i = 0; i < baibangangList.Length; i++)
	        {
	            itemTemp = Instantiate(Resources.Load("Prefab/ThrowCard/TopAndBottomCard")) as GameObject;
	            itemTemp.transform.parent = gangParther.transform;
	            itemTemp.transform.localScale = Vector3.one * 0.8f;
	            itemTemp.GetComponent<TopAndBottomCardScript>().setPoint(int.Parse(baibangangList[i]));
	        }
	    }
