...
public class GameOverScript : MonoBehaviour {
	...
	void Start () {
   //     SocketEventHandle.getInstance().huataitiaoResponse += setHuaAndTiaoAndTai;
	}
	...
	public void setDisplaContent(int dispalyFlag,List<AvatarVO> personList,string allMas,List<int> validMas){
		...
		if (dispalyFlag == 0) {
          ...
          //  getMas (allMas);  //得到抓马的东西
            setSignalContent();//封装实例化 具体 赢点面板
	...
	private void initRoomBaseInfo(){
        ...
        else if (GlobalDataScript.roomVo.roomType == GameConfig.GAME_TYPE_WENZHOU)
        {
             title.text = "乐清麻将";
        }
...
*/
	...
	private void setSignalContent(){
		...
		if (GlobalDataScript.hupaiResponseVo != null && GlobalDataScript.hupaiResponseVo.avatarList.Count > 0) {
			for (int i = 0; i < GlobalDataScript.hupaiResponseVo.avatarList.Count; i++) {
                ...
                itemTemp.GetComponent<SignalOverItemScript>().SetGangTouBaiBanGang(itemdata, mAvatarvoList);
			}
		} 
	}
	...
	public void reStratGame(){
		...
		if (GlobalDataScript.surplusTimes > 0||GlobalDataScript.roomVo.isGoldRoom) {
