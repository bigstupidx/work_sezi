...
public class CrateRoomSettingScript : MonoBehaviour {
    ...
    public Toggle huangjintoggle, daifengtoggle;
...
//        }
		//SocketEventHandle.getInstance ().enterRoomResponse += onCreateRoomCallback;//enterRoomResponse;
	 ...
	 * 创建广东麻将房间
	 */
    //public void createGuanDongRoom()
    //{
    //    SoundCtrl.getInstance().playSoundByActionButton(1);
    ...
    //        {
    //            //maCount = 2 * (i + 1);
    //            maCount = i >= 2 ? 4 * i : 2 * (i + 1);
    //            break;
    //        }
    //    }
    //    sendVo = new RoomCreateVo();
    //    sendVo.ma = maCount;
    //    MyDebug.Log("macount*******************************" + maCount);
    ...
    public List<Toggle> jushu;
    public List<Toggle> renshu;
    public List<Toggle> wanfa;
    //外部调用 创建温州房间按钮
    public void CreatRoom()
    {
        ...
        int roundNumber = 8;
        int wanfahua = 0;
        for (int i = 0; i < jushu.Count; i++)
        {
            Toggle itemjushu = jushu[i];
            if (itemjushu.isOn)
            {
                switch (i)
                {
        ...
        sendVo.roomTiao = roomtiao;
        sendVo.huaType = wanfahua;
        //sendVo.ma = maCount;
        ...
        sendVo.hong = true;
        sendVo.ziMo = 0; //可抢杠胡
        sendVo.huangjinDouble = 0; //可黄金7对
        sendVo.isGoldRoom = false; //是否为金币场
		sendVo.gametype = GameConfig.GAME_TYPE_SEZI;

        GlobalDataScript.isGoldQuickStar = false;
	...
	public void createZhuanzhuanRoom(){
		...
		int roundNumber = 4;//房卡数量
		bool isZimo=false;//自摸
		bool hasHong=false;//红中赖子 //白板癞子
		bool isSevenDoube =false;//七小对
        bool isFollowBank = false; // 是否分猪肉
        bool isHuangJinSevenDouble = false;
        bool isHaveFen=false;
		//bool isGang = false;
		int maCount = 0;
		for (int i = 0; i < zhuanzhuanRoomCards.Count; i++) {
			Toggle item = zhuanzhuanRoomCards [i];
        ...
        if (zhuanzhuanGameRule[4].isOn)
        {
            isFollowBank = true;
        }
        if (huangjintoggle.isOn)
        {
            isHuangJinSevenDouble = true;
        }
        if (daifengtoggle.isOn)
        {
            isHaveFen = true;
        }
		for (int i = 0; i < zhuanzhuanZhuama.Count; i++) {
		...
		sendVo.hong = hasHong;
		sendVo.sevenDouble = isSevenDoube;
        sendVo.followBank = isFollowBank ? 1 : 0;
        sendVo.huangjinDouble = isHuangJinSevenDouble ? 1:0;
        sendVo.isHavaFen = isHaveFen ? 0:1;
		//sendVo.roomType = GameConfig.GAME_TYPE_ZHUANZHUAN;
       // sendVo.roomType = GameConfig.GAME_TYPE_GUANGDONG;
        sendVo.roomType = GameConfig.GAME_TYPE_WENZHOU;
        MyDebug.Log("白板做鬼==="+hasHong+"是否七对====="+isSevenDoube+"是否自模糊==="+isZimo+"分猪肉="+isFollowBank+"黄金7对"+isHuangJinSevenDouble+"是否带风"+isHaveFen);
		string sendmsgstr = JsonMapper.ToJson (sendVo);
		if (GlobalDataScript.loginResponseData.account.roomcard > 0) {
            watingPanel.gameObject.SetActive(true);
...
//	}
	...
	public void onCreateRoomCallback(ClientResponse response){
		...
		//GlobalDataScript.gamePlayPanel.GetComponent<SeZiLogicScript> ().createRoomAddAvatarVO (GlobalDataScript.loginResponseData);
