...
public class CustomSocket{
    ...
    public void Connect()
    {
        ...
        try
        {
            ...
            tcpclient.NoDelay = true;
                        //                                     端口
            tcpclient.BeginConnect(APIS.socketUrl, 10007, new AsyncCallback(ConnectCallback), tcpclient);
        }
        catch(Exception ex)
        {
            //设置标志,连接服务端失败!
	...
	private void readBuffer(BinaryReader buffers){
        ...
        if (headcode == 201)
        {
            status = ReadInt(buffers.ReadBytes(4));
          //  messageLen = ReadShort(buffers.ReadBytes(2));
             soundLen = ReadInt(buffers.ReadBytes(4));
        }
        else 
        {
             status = ReadInt(buffers.ReadBytes(4));
		...
		if(flag == 1){
            ...
            if (headcode == 201)
            {
                response.uuidOnlyforChatMic = ReadInt(buffers.ReadBytes(4));
                Debug.Log("uuid=" + response.uuidOnlyforChatMic);
            ...
            {
                message = Encoding.UTF8.GetString(buffers.ReadBytes(messageLen));
